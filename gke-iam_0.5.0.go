...
func NewGkeIam050(
    ...
    var componentResource GkeIam050
...
_, err = castai.GkeUserPolicies(ctx, map[string]interface{}{
}, nil);
if err != nil {
return err
}
...
err =     ctx.RegisterResourceOutputs(&componentResource, pulumi.Map{
        "privateKey": %!v(PANIC=Format method: fatal: A failure has occurred: unlowered conditional expression @ output.pp:1,11-3,17),
        "serviceAccountId": %!v(PANIC=Format method: fatal: A failure has occurred: unlowered conditional expression @ output.pp:7,11-72),
        "serviceAccountEmail": %!v(PANIC=Format method: fatal: A failure has occurred: unlowered conditional expression @ output.pp:11,11-68),
...
"compute.instances.use",
    ...
    componentResource.PrivateKey = %!v(PANIC=Format method: fatal: A failure has occurred: unlowered conditional expression @ output.pp:1,11-3,17)
    componentResource.ServiceAccountId = %!v(PANIC=Format method: fatal: A failure has occurred: unlowered conditional expression @ output.pp:7,11-72)
    componentResource.ServiceAccountEmail = %!v(PANIC=Format method: fatal: A failure has occurred: unlowered conditional expression @ output.pp:11,11-68)
